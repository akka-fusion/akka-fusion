...
class MqttCodecSpec extends WordSpec with Matchers {
  ...
  "the codec" should {
    ...
    "invalid packet size when decoding control packets" in {
      ByteString.newBuilder.putByte(0x00).putByte(0x01).result().iterator.decodeControlPacket(0) shouldBe Left(
        MqttCodec.InvalidPacketSize(1, 0))
    }
    ...
    "unknown protocol name/level when decoding connect control packets" in {
      val bsb = ByteString.newBuilder.putByte((ControlPacketType.CONNECT.underlying << 4).toByte).putByte(2)
      "blah".encode(bsb)
      bsb.putByte(0)
      bsb.result().iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(
        MqttCodec.UnknownConnectProtocol(Right("blah"), 0))
    }

    "connect flag reserved set when decoding connect control packets" in {
      val bsb = ByteString.newBuilder.putByte((ControlPacketType.CONNECT.underlying << 4).toByte).putByte(3)
      Connect.Mqtt.encode(bsb)
      bsb.putByte(Connect.v311.toByte)
      bsb.putByte(ConnectFlags.Reserved.underlying.toByte)
      bsb.result().iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(MqttCodec.ConnectFlagReservedSet)
    }

    "bad connect message when decoding connect control packets" in {
      val bsb = ByteString.newBuilder.putByte((ControlPacketType.CONNECT.underlying << 4).toByte).putByte(26)
      Connect.Mqtt.encode(bsb)
      bsb.putByte(Connect.v311.toByte)
      ...
      bsb.result().iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(
        MqttCodec.BadConnectMessage(
          Right("some-client-id"),
    ...
    "reserved bits set when decoding connect ack packets" in {
      val bsb = ByteString.newBuilder.putByte((ControlPacketType.CONNACK.underlying << 4).toByte).putByte(2)
      bsb.putByte(2)
      bsb.putByte(0)
      bsb.result().iterator.decodeConnAck() shouldBe Left(MqttCodec.ConnectAckFlagReservedBitsSet)
    }
    ...
    "invalid QoS when decoding publish packets" in {
      val bsb = ByteString.newBuilder
        ...
        .putByte(0)
      bsb.result().iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(MqttCodec.InvalidQoS)
    }

    "bad publish message when decoding publish packets" in {
      val bsb = ByteString.newBuilder.putByte((ControlPacketType.PUBLISH.underlying << 4).toByte).putByte(0)
      bsb.result().iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(
    ...
    "underflow when decoding publish packets" in {
      ...
      bsb.result().iterator.decodePublish(0, ControlPacketFlags.QoSAtLeastOnceDelivery) shouldBe Left(
        MqttCodec.BufferUnderflow)
    }
    ...
    "bad subscribe message when decoding subscribe packets given bad QoS" in {
      ...
      bytes.iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(
        BadSubscribeMessage(
          PacketId(1),
          List(
    ...
    "bad subscribe message when decoding subscribe packets given no topics" in {
      ...
      val bytes = packet.encode(bsb, PacketId(1)).result()
      bytes.iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(BadSubscribeMessage(PacketId(1), List.empty))
    }
    ...
    "regular sub ack message when decoding sub ack packets given failure QoS" in {
      ...
      val bytes = packet.encode(bsb).result()
      bytes.iterator.decodeControlPacket(MaxPacketSize) shouldBe Right(
    ...
    "bad unsubscribe message when decoding unsubscribe packets given no topics" in {
      ...
      val bytes = packet.encode(bsb, PacketId(1)).result()
      bytes.iterator.decodeControlPacket(MaxPacketSize) shouldBe Left(BadUnsubscribeMessage(PacketId(1), List.empty))
    }
