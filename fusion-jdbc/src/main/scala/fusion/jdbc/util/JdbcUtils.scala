...
object JdbcUtils extends StrictLogging {
  private val BeanIgnoreClass = classOf[BeanIgnore]
  // Check for JDBC 4.1 getObject(int, Class) method - available on JDK 7 and higher

  private val getObjectWithTypeAvailable = hasMethod(classOf[ResultSet], "getObject", classOf[Int], classOf[Class[_]])

  private def hasMethod(clazz: Class[_], methodName: String, paramTypes: Class[_]*): Boolean = {
    def getMethodIfAvailable(clazz: Class[_], methodName: String, paramTypes: Class[_]*): Method = {
      require(clazz ne null, "Class must not be null")
      require(methodName ne null, "Method name must not be null")
      if (paramTypes != null) {
        try {
          clazz.getMethod(methodName, paramTypes: _*)
        } catch {
...
//            logger.warn(s"获取parameterTypes异常；${e.toString}")
            Nil
        }
