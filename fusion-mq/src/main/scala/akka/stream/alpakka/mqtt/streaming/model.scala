 ...
 * http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html
 */
final case class Publish @InternalApi private[streaming] (
 ...
 * http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html
 */
final case class Subscribe @InternalApi private[streaming] (
    packetId: PacketId,
...
object MqttCodec {
   ...
   */
  final case class BadConnectMessage(
      clientId: Either[MqttCodec.DecodeError, String],
   ...
   */
  final case class BadPublishMessage(
      topicName: Either[DecodeError, String],
      packetId: Option[PacketId],
      payload: ByteString)
      extends DecodeError {
    override def toString: String =
   ...
   */
  final case class BadSubscribeMessage(
      packetId: PacketId,
  ...
  implicit class MqttByteIterator(val v: ByteIterator) extends AnyVal {
    ...
    def decodeUnsubscribe(l: Int): Either[DecodeError, Unsubscribe] =
      try {
        ...
        def decodeTopicFilters(
            remainingLen: Int,
            topicFilters: Vector[Either[DecodeError, String]]): Vector[Either[DecodeError, String]] =
          if (remainingLen > 0) {
            val packetLenAtTopicFilter = v.len
...
final case class Command[A](command: ControlPacket, completed: Option[Promise[Done]], carry: Option[A]) {
  ...
  def this(command: ControlPacket, completed: Optional[CompletionStage[Done]], carry: Optional[A]) =
    this(command, completed.asScala.map { f =>
      val p = Promise[Done]
      p.future.foreach(f.toCompletableFuture.complete)(ExecutionContext.fromExecutorService(ForkJoinPool.commonPool()))
      p
    }, carry.asScala)
