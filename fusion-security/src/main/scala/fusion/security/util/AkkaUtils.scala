...
object AkkaUtils {
  ...
  )(implicit system: ActorSystem[_], timeout: Timeout): Future[Set[ActorRef[T]]] = {
  ...
  def receptionistFindOne[T](
      serviceKey: ServiceKey[T]
  )(implicit system: ActorSystem[_], timeout: Timeout): Future[ActorRef[T]] = {
    implicit val ec = system.executionContext
  ...
  def receptionistFindSetSync[T](
      serviceKey: ServiceKey[T]
  )(implicit system: ActorSystem[_], timeout: Timeout): Set[ActorRef[T]] = {
    Await.result(receptionistFindSet(serviceKey), timeout.duration)
  }

  def receptionistFindOneSync[T](
      serviceKey: ServiceKey[T]
  )(implicit system: ActorSystem[_], timeout: Timeout): ActorRef[T] = {
