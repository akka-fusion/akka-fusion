...
@InternalApi private[streaming] object ServerConnector {
  ...
  final case class ConnectReceivedFromRemote(
      override val connectionId: ByteString,
      connect: Connect,
  ...
  final case class ConnAckReceivedLocally(
      override val connectionId: ByteString,
      connAck: ConnAck,
      remote: Promise[Source[ClientConnection.ForwardConnAckCommand, NotUsed]])
      extends Event(connectionId)
  final case class SubscribeReceivedFromRemote(
      override val connectionId: ByteString,
      subscribe: Subscribe,
      local: Promise[Publisher.ForwardSubscribe.type])
      extends Event(connectionId)
  final case class PublishReceivedFromRemote(
      override val connectionId: ByteString,
      publish: Publish,
  ...
  final case class UnsubscribeReceivedFromRemote(
      override val connectionId: ByteString,
      unsubscribe: Unsubscribe,
      local: Promise[Unpublisher.ForwardUnsubscribe.type])
      extends Event(connectionId)
  final case class PingReqReceivedFromRemote(
      override val connectionId: ByteString,
      local: Promise[ClientConnection.ForwardPingReq.type])
      extends Event(connectionId)
  final case class DisconnectReceivedFromRemote(
      override val connectionId: ByteString,
  ...
  final case class QueueOfferCompleted(
      override val connectionId: ByteString,
    ...
    Behaviors
      .receiveMessagePartial[Event] {
        case ConnectReceivedFromRemote(connectionId, connect, local) =>
          ...
          listening(data.copy(clientConnections = data.clientConnections.filterNot {
              case (_, (clientId, _)) => clientId == connect.clientId
            } + newConnection))
      ...
      Behaviors
        .receivePartial[Event] {
          case (context, ConnAckReceivedLocally(_, remote)) =>
            ...
            data.activeProducers.values.foreach(_ ! Producer.ReceiveConnect)

            QueueOfferState.waitForQueueOfferCompleted(
              queue.offer(ForwardConnAck),
              result => QueueOfferCompleted(result.toEither),
              clientConnected(ConnAckReplied(
                data.connect,
                queue,
                Vector.empty,
                data.publishers,
      ...
      Behaviors
        .receivePartial[Event] {
          ...
          case (_, Subscribed(subscribe)) =>
            clientConnected(data.copy(publishers = data.publishers ++ subscribe.topicFilters.map(_._1)))
          ...
          case (context, prl @ PublishReceivedLocally(publish, publishData))
            ...
            if (!data.activeProducers.contains(publish.topicName)) {
              ...
              val producer =
                context
                  .spawn(Producer(publish, publishData, reply, data.producerPacketRouter, data.settings), producerName)
...
@InternalApi private[streaming] object Publisher {
  ...
  final case class ServerSubscribe(
      override val clientId: Some[String],
      override val packetId: PacketId,
      override val subscribed: Promise[Done],
      override val packetRouter: ActorRef[RemotePacketRouter.Request[Event]],
