...
trait STMultiNodeSpec
  ...
  def typedSystem: ActorSystem[_] = schedulerX.system
...
abstract class SchudulerXMultiNodeSpec(nodeConfig: MultiNodeConfig, protected val schedulerX: SchedulerX)
    extends MultiNodeSpec(nodeConfig.myself, schedulerX.classicSystem, nodeConfig.roles, nodeConfig.deployments) {
  def this(nodeConfig: MultiNodeConfig, create: Config => SchedulerX) {
