...
@InternalApi private[streaming] object ClientConnector {
  ...
  final case class DisconnectReceivedLocally(
      override val connectionId: ByteString,
      remote: Promise[ForwardDisconnect.type])
      extends Event(connectionId)

  final case class SubscribeReceivedLocally(
      override val connectionId: ByteString,
      subscribe: Subscribe,
  ...
  final case class PingRespReceivedFromRemote(
      override val connectionId: ByteString,
  ...
  final case class UnsubscribeReceivedLocally(
      override val connectionId: ByteString,
      ...
      Behaviors
        .receivePartial[Event] {
          ...
          case (context, ConnAckReceivedFromRemote(_, connAck, local))
            ...
            BehaviorRunner.run(
              serverConnected(ConnAckReceived(
                data.connectionId,
  ...
  def serverConnected(data: ConnAckReceived, resetPingReqTimer: Boolean = true)(
      implicit mat: Materializer): Behavior[Event] =
          ...
          case (
            ...
            data.activeConsumers.get(topicName) match {
              case None =>
                ...
                serverConnected(
                  data.copy(activeConsumers = data.activeConsumers + (publish.topicName -> consumer)),
  ...
  def prepareServerSubscribe(data: Start): Behavior[Event] = Behaviors.setup { context =>
    ...
    Behaviors.receiveMessagePartial[Event] {
      case AcquiredPacketId(packetId) =>
        ...
        serverSubscribe(ServerSubscribe(packetId, data.subscribeData, data.packetRouter, data.settings))
  ...
  def prepareServerUnsubscribe(data: Start): Behavior[Event] = Behaviors.setup { context =>
    ...
    Behaviors.receiveMessagePartial[Event] {
      case AcquiredPacketId(packetId) =>
        ...
        serverUnsubscribe(ServerUnsubscribe(packetId, data.unsubscribeData, data.packetRouter, data.settings))
      case UnobtainablePacketId =>
