...
class MqttSessionSpec
  ...
  "MQTT client connector" should {
    ...
    "flow through a client session" in assertAllStagesStopped {
      ...
      val (client, result) =
        Source
          ...
          .via(Mqtt.clientSessionFlow(session, ByteString("1")).join(pipeToServer))
          .take(3)
    ...
    "correctly handle a new client connection" in assertAllStagesStopped {
      ...
      val (secondClient, secondResult) =
        Source
          .queue[Command[String]](1, OverflowStrategy.fail)
          .via(Mqtt.clientSessionFlow(session, ByteString("2")).join(pipeToServer))
          .toMat(Sink.head)(Keep.both)
          .run()
    ...
    "receive a QoS 1 publication with DUP indicated from a unsubscribed topic - simulates a reconnect" in assertAllStagesStopped {
      ...
      val publish = Publish(
        ControlPacketFlags.QoSAtLeastOnceDelivery | ControlPacketFlags.DUP,
        "some-topic",
    ...
    "publish and carry through an object to pubAck" in assertAllStagesStopped {
      ...
      val (client, result) =
        Source
          .queue(1, OverflowStrategy.fail)
          .via(Mqtt.clientSessionFlow[String](session, ByteString("1")).join(pipeToServer))
          .drop(1)
          .toMat(Sink.head)(Keep.both)
          .run()
  ...
  "MQTT server connector" should {
    ...
    "flow through a server session" in assertAllStagesStopped {
      ...
      val (fromClientQueue, fromClient) =
        Source.queue[ByteString](1, OverflowStrategy.dropHead).toMat(BroadcastHub.sink)(Keep.both).run()
      ...
      val (server, result) =
        Source
          ...
          .via(Mqtt.serverSessionFlow(session, ByteString.empty).join(pipeToClient))
    ...
    "receive two subscriptions for the same topic" in assertAllStagesStopped {
      ...
      val toClient = Sink.foreach[ByteString](bytes => client.ref ! bytes)
      val (fromClientQueue, fromClient) =
        Source.queue[ByteString](2, OverflowStrategy.dropHead).toMat(BroadcastHub.sink)(Keep.both).run()
    ...
    def reconnectTest(explicitDisconnect: Boolean): Unit = {
      ...
      def server(connectionId: ByteString)
        ...
        val connection = Source
          .queue[Command[Nothing]](1, OverflowStrategy.fail)
          .via(Mqtt.serverSessionFlow(session, connectionId).join(pipeToClient))
          .wireTap(Sink.foreach[Either[DecodeError, Event[_]]] {
    ...
    "produce a duplicate publish on the server given two client connections" in assertAllStagesStopped {
      ...
      val (client1Connection, fromClient1) =
        Source.queue[ByteString](1, OverflowStrategy.dropHead).toMat(BroadcastHub.sink)(Keep.both).run()
      ...
      val (client2Connection, fromClient2) =
        Source.queue[ByteString](1, OverflowStrategy.dropHead).toMat(BroadcastHub.sink)(Keep.both).run()
      ...
      val serverConnection1 =
        Source
          .queue[Command[Nothing]](1, OverflowStrategy.fail)
          .via(Mqtt.serverSessionFlow(serverSession, ByteString.empty).join(pipeToClient1))
          .wireTap(Sink.foreach[Either[DecodeError, Event[_]]] {
            case Right(Event(`connect`, _)) =>
      ...
      val serverConnection2 =
        Source
          .queue[Command[Nothing]](1, OverflowStrategy.fail)
          .via(Mqtt.serverSessionFlow(serverSession, ByteString.empty).join(pipeToClient2))
          .wireTap(Sink.foreach[Either[DecodeError, Event[_]]] {
            case Right(Event(`connect`, _)) =>
